...
func (siw *ServerInterfaceWrapper) CatalogGet(c *gin.Context) {
	...
	if valueList, found := headers[http.CanonicalHeaderKey("X-Broker-API-Version")]; found {
		...
		err = runtime.BindStyledParameterWithOptions("simple", "X-Broker-API-Version", valueList[0], &XBrokerAPIVersion, runtime.BindStyledParameterOptions{ParamLocation: runtime.ParamLocationHeader, Explode: false, Required: true})
		if err != nil {
			siw.ErrorHandler(c, fmt.Errorf("Invalid format for parameter X-Broker-API-Version: %w", err), http.StatusBadRequest)
			return
		}
		...
		siw.ErrorHandler(c, fmt.Errorf("Header parameter X-Broker-API-Version is required, but not found"), http.StatusBadRequest)
		return
	}
...
func (siw *ServerInterfaceWrapper) ServiceInstanceDeprovision(c *gin.Context) {
	...
	err = runtime.BindStyledParameterWithOptions("simple", "instance_id", c.Param("instance_id"), &instanceId, runtime.BindStyledParameterOptions{Explode: false, Required: true})
	if err != nil {
	...
	if paramValue := c.Query("plan_id"); paramValue != "" {
		...
		siw.ErrorHandler(c, fmt.Errorf("Query argument plan_id is required, but not found"), http.StatusBadRequest)
		return
	}

	err = runtime.BindQueryParameter("form", true, true, "plan_id", c.Request.URL.Query(), &params.PlanId)
	if err != nil {
		siw.ErrorHandler(c, fmt.Errorf("Invalid format for parameter plan_id: %w", err), http.StatusBadRequest)
		return
	}
	...
	if err != nil {
		siw.ErrorHandler(c, fmt.Errorf("Invalid format for parameter accepts_incomplete: %w", err), http.StatusBadRequest)
		return
	}
	...
	if valueList, found := headers[http.CanonicalHeaderKey("X-Broker-API-Originating-Identity")]; found {
		...
		err = runtime.BindStyledParameterWithOptions("simple", "X-Broker-API-Originating-Identity", valueList[0], &XBrokerAPIOriginatingIdentity, runtime.BindStyledParameterOptions{ParamLocation: runtime.ParamLocationHeader, Explode: false, Required: false})
		if err != nil {
			siw.ErrorHandler(c, fmt.Errorf("Invalid format for parameter X-Broker-API-Originating-Identity: %w", err), http.StatusBadRequest)
			return
		}
